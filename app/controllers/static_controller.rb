...
class StaticController < ApplicationController
  ...
  def index
    ...
    # puts "\n"
    # puts Dir.entries(path+'/client/src/app').select {|entry| File.directory? File.join(path+'/client/dist/prod',entry) and !(entry =='.' || entry == '..') }
    
    # file_paths = Find.find(Rails.root).select { |p| /.*ngsw.json$/ =~ p }
    # puts "\n\n\n"+file_paths.to_yaml+"\n\n\n"

    render file: path+'/client/dist/prod/index.html'
  end
end
